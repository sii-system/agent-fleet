...
if declare -p OPIK_URL >/dev/null 2>&1; then
  run_env+=("OPIK_URL=${OPIK_URL:-}")
fi
for optional in PI_VERSION PI_PROVIDER PI_THINKING_LEVEL OPIK_API_KEY OPIK_WORKSPACE OPIK_PROJECT_NAME MIN_TEST MIN_TEST_INCLUDE_TASK CLAUDE_TGZ_SOURCE CLAUDE_WHEEL_DIR_SOURCE HARBOR_CC_CLAUDE_TGZ_SOURCE HARBOR_CC_PY_WHEEL_DIR_SOURCE; do
  if [[ -n "${!optional:-}" ]]; then
...
for optional in PIP_INDEX_URL PIP_EXTRA_INDEX_URL PIP_TRUSTED_HOST NPM_CONFIG_REGISTRY GO111MODULE GOPROXY GOSUMDB RUSTUP_UPDATE_ROOT RUSTUP_DIST_SERVER CARGO_REGISTRY_REPLACE_WITH CARGO_REGISTRY_URL DIND_REGISTRY_MIRRORS DIND_REGISTRY_MIRROR HARBOR_SKIP_DOCKERHUB_PREFLIGHT; do
  if [[ -n "${!optional:-}" ]]; then
    run_env+=("$optional=${!optional}")
  fi
done
if [[ -n "$registry_mirrors" && -z "${HARBOR_SKIP_DOCKERHUB_PREFLIGHT:-}" ]]; then
  run_env+=("HARBOR_SKIP_DOCKERHUB_PREFLIGHT=1")
fi
