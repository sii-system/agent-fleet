...
run_env+=("${proxy_env[@]}")
for optional in PI_VERSION TRACE_TO_OPIK OPIK_URL OPIK_API_KEY OPIK_WORKSPACE OPIK_PROJECT_NAME MIN_TEST MIN_TEST_INCLUDE_TASK CLAUDE_TGZ_SOURCE CLAUDE_WHEEL_DIR_SOURCE HARBOR_CC_CLAUDE_TGZ_SOURCE HARBOR_CC_PY_WHEEL_DIR_SOURCE; do
  if [[ -n "${!optional:-}" ]]; then
