...
is available, the source is downloaded through the Dependency Gateway when
`DEPENDENCY_GATEWAY_URL` is set, and falls back to the pinned release URL when
the gateway is unset or unreachable; the digest is verified before use.
