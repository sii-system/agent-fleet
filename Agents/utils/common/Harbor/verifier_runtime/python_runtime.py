...
def _static_source_urls() -> list[str]:
    """Return the ordered download URLs for the pinned source archive.

    The Dependency Gateway is preferred when DEPENDENCY_GATEWAY_URL is set;
    the pinned release URL is always the final fallback.
    """
    urls = []
    gateway = os.environ.get("DEPENDENCY_GATEWAY_URL", "").rstrip("/")
    if gateway:
        ...
        if (
            ...
            raise ValueError("DEPENDENCY_GATEWAY_URL must be credential-free HTTP(S)")
        upstream = urlsplit(STATIC_SOURCE["url"])
        urls.append(
            f"{gateway}/download/v1/{upstream.scheme}/"
            f"{upstream.netloc.encode().hex()}/object/{upstream.path.encode().hex()}"
        )
    urls.append(str(STATIC_SOURCE["url"]))
    return urls


def _static_source_archive(cache_dir: Path) -> Path:
    """Resolve the pinned source from an override, cache, or download.
    ...
    When the preferred download fails (for example an unreachable gateway),
    the next URL in _static_source_urls() is tried.
    """
    ...
    temporary = Path(name)
    try:
        urls = _static_source_urls()
        for index, url in enumerate(urls):
            try:
                print(
                    f"[prepare] downloading pinned static verifier Python: {url}",
                    flush=True,
                )
                ...
                if index + 1 == len(urls):
                    raise
                print(
                    f"[prepare] source download failed: {exc}; "
                    f"falling back to {urls[index + 1]}",
                    flush=True,
                )
    finally:
        temporary.unlink(missing_ok=True)
    raise RuntimeError("verifier runtime error: no source URL available")
