...
HARBOR_BENCHMARK_NAME="${HARBOR_BENCHMARK_NAME:-}"
# Optional newline-delimited KEY=value file. Values are forwarded to the agent
# environment without evaluating shell syntax. This is the generic benchmark
# integration point for service endpoints and non-secret run metadata.
HARBOR_AGENT_ENV_FILE="${HARBOR_AGENT_ENV_FILE:-}"
# Optional Claude-style .mcp.json or Harbor MCP config. Harbor translates this
# into the selected agent's native MCP configuration when it supports MCP.
HARBOR_MCP_CONFIG="${HARBOR_MCP_CONFIG:-}"
...
export HARBOR_N_CONCURRENT HARBOR_MAX_RETRIES HARBOR_RETRY_INCLUDE_EXCEPTIONS HARBOR_RETRY_EXCLUDE_EXCEPTIONS HARBOR_AK_MAX_TURNS HARBOR_AK_COLLECT_ROLLOUT_DETAILS HARBOR_AK_ENABLE_SUMMARIZE HARBOR_BENCHMARK_NAME HARBOR_AGENT_ENV_FILE HARBOR_MCP_CONFIG HARBOR_DISALLOWED_TOOLS HARBOR_APPEND_SYSTEM_PROMPT
...
export RL_AGENT RL_MODEL_NAME RL_MODEL_PREFIX RL_API_BASE RL_API_KEY RL_API_KEY_MODE MODEL_REQUEST_CONFIG_JSON RL_DISABLED_TASK_IDS RL_BENCHMARK RL_RESULT_PROCESSOR RL_ENVIRONMENT_TYPE RL_E2B_SANDBOX_TIMEOUT_SEC RL_E2B_PREBUILT_TEMPLATE RL_FORCE_BUILD
