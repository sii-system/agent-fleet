...
dist_dir="${MIMO_ROUTER_DIST_DIR:-${OUTPUT_ROOT}/.mimo-router/$router_version-$router_short}"
mkdir -p "$dist_dir"
...
MIMO_ROUTER_CONFIG="$dist_dir/router-config-mimo-max.json"
python3 - "$source_config" "$MIMO_ROUTER_CONFIG" <<'PY'
import json
import sys
...
payload.setdefault("routing", {})["max_fusions"] = -1
...
mimo = payload.setdefault("mimo_max", {})
mimo.update(
    {
        "model": "sonnet",
...
printf '[mimo-code] router=%s@%s\n' "$router_version" "$router_short"
