...
_USER_SUBAGENT_MODEL_SET="${HARBOR_CLAUDE_CODE_SUBAGENT_MODEL+x}"
_USER_SUBAGENT_MODEL_VALUE="${HARBOR_CLAUDE_CODE_SUBAGENT_MODEL-}"
_USER_ANTHROPIC_MODEL_SET="${HARBOR_ANTHROPIC_MODEL+x}"
_USER_ANTHROPIC_MODEL_VALUE="${HARBOR_ANTHROPIC_MODEL-}"
_USER_ANTHROPIC_OPUS_MODEL_SET="${HARBOR_ANTHROPIC_DEFAULT_OPUS_MODEL+x}"
_USER_ANTHROPIC_OPUS_MODEL_VALUE="${HARBOR_ANTHROPIC_DEFAULT_OPUS_MODEL-}"
_USER_ANTHROPIC_SONNET_MODEL_SET="${HARBOR_ANTHROPIC_DEFAULT_SONNET_MODEL+x}"
_USER_ANTHROPIC_SONNET_MODEL_VALUE="${HARBOR_ANTHROPIC_DEFAULT_SONNET_MODEL-}"
_USER_ANTHROPIC_HAIKU_MODEL_SET="${HARBOR_ANTHROPIC_DEFAULT_HAIKU_MODEL+x}"
_USER_ANTHROPIC_HAIKU_MODEL_VALUE="${HARBOR_ANTHROPIC_DEFAULT_HAIKU_MODEL-}"
...
export HARBOR_RUNS="${HARBOR_RUNS:-1}"
export MAX_RETRIES="${MAX_RETRIES:-0}"
export HARBOR_MAX_RETRIES="${HARBOR_MAX_RETRIES:-0}"
...
if [[ "$_USER_SUBAGENT_MODEL_SET" == "x" ]]; then
  HARBOR_CLAUDE_CODE_SUBAGENT_MODEL="$_USER_SUBAGENT_MODEL_VALUE"
else
...
export HARBOR_MODEL="$MAIN_MODEL"
if [[ "$_USER_ANTHROPIC_MODEL_SET" == "x" ]]; then
  HARBOR_ANTHROPIC_MODEL="$_USER_ANTHROPIC_MODEL_VALUE"
else
  HARBOR_ANTHROPIC_MODEL="$MAIN_MODEL"
fi
if [[ "$_USER_ANTHROPIC_OPUS_MODEL_SET" == "x" ]]; then
  HARBOR_ANTHROPIC_DEFAULT_OPUS_MODEL="$_USER_ANTHROPIC_OPUS_MODEL_VALUE"
else
  HARBOR_ANTHROPIC_DEFAULT_OPUS_MODEL="$MAIN_MODEL"
fi
if [[ "$_USER_ANTHROPIC_SONNET_MODEL_SET" == "x" ]]; then
  HARBOR_ANTHROPIC_DEFAULT_SONNET_MODEL="$_USER_ANTHROPIC_SONNET_MODEL_VALUE"
else
  HARBOR_ANTHROPIC_DEFAULT_SONNET_MODEL="$MAIN_MODEL"
fi
if [[ "$_USER_ANTHROPIC_HAIKU_MODEL_SET" == "x" ]]; then
  HARBOR_ANTHROPIC_DEFAULT_HAIKU_MODEL="$_USER_ANTHROPIC_HAIKU_MODEL_VALUE"
else
  HARBOR_ANTHROPIC_DEFAULT_HAIKU_MODEL="$MAIN_MODEL"
fi
export HARBOR_ANTHROPIC_MODEL
export HARBOR_ANTHROPIC_DEFAULT_OPUS_MODEL
export HARBOR_ANTHROPIC_DEFAULT_SONNET_MODEL
export HARBOR_ANTHROPIC_DEFAULT_HAIKU_MODEL
export HARBOR_CLAUDE_CODE_SUBAGENT_MODEL
...
export HARBOR_FUSION_ROUND_ROUTER_DIR="$FUSION_ROUTER_DIR/src/sii_fusion_router/frontends/claude_code"
export HARBOR_FUSION_TASK_BUILDER="$HARBOR_FUSION_ROUND_ROUTER_DIR/task_subagent_prompt.py"
export HARBOR_FUSION_ROUND_ROUTER_MOUNT_PATH="/opt/tb-fusion-round"
export HARBOR_FUSION_ROUND_GATE=1
export HARBOR_FUSION_ROUND_GATE_PATH="/opt/tb-fusion-round/subagent_barrier_gate.py"
export HARBOR_FUSION_ROUND_GATE_MODE=mid-turn-fusion
export HARBOR_FUSION_MAX_FUSIONS_PER_TASK="${HARBOR_FUSION_MAX_FUSIONS_PER_TASK:-1}"
export HARBOR_FUSION_PANEL_CALL_BUDGET="${HARBOR_FUSION_PANEL_CALL_BUDGET:-}"
...
for required_path in \
  "$FUSION_ROUTER_DIR" \
  "$HARBOR_FUSION_ROUND_ROUTER_DIR" \
  "$HARBOR_FUSION_TASK_BUILDER" \
  "$FUSION_ROUTER_DIR/prompts/mid_turn_fusion/panel.md" \
  "$FUSION_ROUTER_DIR/prompts/mid_turn_fusion/outer.md" \
  "$HARBOR_FUSION_ROUND_ROUTER_DIR/subagent_barrier_gate.py" \
  "$HARBOR_FUSION_ROUND_ROUTER_DIR/templates"; do
...
export HARBOR_FUSION_TASK_FILE_SOURCE="$task_file"
export HARBOR_FUSION_TASK_FILE="/opt/tb-fusion-task/$(basename "$task_file")"

python3 "$HARBOR_FUSION_TASK_BUILDER" prepare \
  --task-id "$TASK_ID" \
  --task-file "$task_file" \
...
export HARBOR_CLAUDE_CODE_AGENTS_JSON="$(tr -d '\n' < "$SPAN_TASK_SUBAGENT_AGENTS_FILE")"

filtered_disallowed=""
for tool in ${HARBOR_DISALLOWED_TOOLS//,/ }; do
  if [[ "$tool" == "Task" || "$tool" == "Agent" || "$tool" == "Bash" ]]; then
    continue
  fi
  filtered_disallowed="${filtered_disallowed:+$filtered_disallowed }$tool"
done
export HARBOR_DISALLOWED_TOOLS="$filtered_disallowed"

task_subagent_prompt="$(<"$SPAN_TASK_SUBAGENT_PROMPT_FILE")"
export HARBOR_APPEND_SYSTEM_PROMPT="${HARBOR_APPEND_SYSTEM_PROMPT:-}
...
python3 "$HARBOR_FUSION_TASK_BUILDER" finalize \
  --task-id "$TASK_ID" \
