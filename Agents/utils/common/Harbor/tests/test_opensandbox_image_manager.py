...
def run_apt_frontend_build(**kwargs):
    from opensandbox_buildkit_frontend import prepare_frontend

    kwargs["build_args"] = {
        **kwargs["build_args"],
        **prepare_frontend(kwargs["secret_files"], kwargs["log_path"].parent / "frontend",
                           build_contexts=kwargs.setdefault("build_contexts", {})),
    }
    run_build(**kwargs)
...
RUN ["/bin/sh", "-c", "grep -F 'deb [trusted=yes] http://original.invalid:18765/repo ./' /etc/apt/sources.list.d/probe.list && test -x /run/opensandbox-apt/bin/apt-get && apt-cache policy opensandbox-index-probe | grep -F '1.0'"]
'''
        rendered = render_build_dockerfile(
            source,
            dockerhub_mirror_prefix="",
