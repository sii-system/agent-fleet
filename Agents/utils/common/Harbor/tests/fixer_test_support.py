    ...
    path.write_text(
        "\n".join(
            [
                ...
                "if payload['kind'] == 'harbor_fixer_policy_agent_input':",
                "    result = {",
                "      'schema_version': 1, 'kind': 'harbor_fixer_policy_agent_decision',",
                "      'tier': payload['tier'], 'plan_id': payload['plan_id'], 'action_id': payload['action']['action_id'],",
                "      'decision': 'allow', 'risk_level': 'medium', 'reason_code': 'fixture_allow', 'reason': 'Fixture policy allows this command.'}",
                "elif payload['kind'] == 'harbor_fixer_task_input':",
