        ...
        async def _run_local_opencode_install() -> None:
            ...
            await self.exec_as_agent(
                ...
                command=(
                    ...
                    "    || return 1; "
                    "  ln -sf \"$runtime_bin/node\" \"$HOME/.local/bin/node\" "
                    "    || return 1; "
                    "  ln -sf \"$runtime_bin/npm\" \"$HOME/.local/bin/npm\" "
                    "    || return 1; "
                    "  if [ -e \"$runtime_bin/npx\" ]; then "
                    "    ln -sf \"$runtime_bin/npx\" \"$HOME/.local/bin/npx\" "
                    "      || return 1; "
                    "  else rm -f \"$HOME/.local/bin/npx\"; fi; "
                    "  hash -r; "
                    "  if node_runtime_ready; then return 0; fi; "
                    ...
                    "  hash -r; "
                    "  for system_node_runtime_bin in /usr/bin /usr/local/bin; do "
                    "    if activate_node_runtime \"$system_node_runtime_bin\"; then break; fi; "
                    "  done; "
                    "fi; "
                    "if ! node_runtime_ready; then "
